...
(deftest parse
  (binding [lark.tree.emit/*ns* (symbol "lark.tree.parse-test")]
    ...
    (are [in-string the-shape]
      ...
      "\n" [:comment-block #_[:newline]]
      "\n\n;A" [:comment-block #_[:newline :newline :comment]]
      ";A" [:comment-block #_[:comment]]
      " ;A" [:comment-block #_[:space :comment]]
      "; a

      ; b" [:comment-block #_[:comment :newline :comment]]
      "a/" [:symbol]
      )
...
(deftest prettify
  (binding [emit/*prettify* true]
    ...
    (are [in out]
      ...
      "[1\n2]" "[1\n 2]"                                    ; not a list
      "(+\n1\n2)" "(+\n  1\n  2)"                           ; nothing on 1st line
      "(+   1\n2)" "(+   1\n     2)"                        ; align with 1st arg by default
      "(let [x 2] 3\n4)" "(let [x 2] 3\n           4)"      ;
      "(let [x 2]\n4)" "(let [x 2]\n  4)"
      "[ {:a 1\n:b 2}]" "[ {:a 1\n   :b 2}]"
      "(->  a\nb" "(->  a\n     b)"
